...
protocol EmailService {
    ...
    func sendPersonalizedEmail(
        to: String, subject: String, markdown: String, mailingList: String, delay: TimeInterval)
        async throws
...
struct IndiePitcherEmailService: EmailService {
    ...
    init(application: Application) {
        ...
        guard let apiKey = Environment.get("IP_SECRET_API_KEY") else {
            fatalError(
                "IP_SECRET_API_KEY env key missing. Create one for free at https://indiepitcher.com or inject the mock version `MockEmailService` in `configure.swift` to run the project without crashing."
            )
        }
