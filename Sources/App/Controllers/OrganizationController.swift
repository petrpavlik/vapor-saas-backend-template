...
extension Request {
    func organization(minRole: ProfileOrganizationRole.Role) async throws -> Organization {
        guard
            let organizationId = self.parameters.get("organizationID").flatMap({
                UUID(uuidString: $0)
            })
        else {
        ...
        guard
            let membership =
                try await ProfileOrganizationRole
                .query(on: self.db)
                .join(
                    Organization.self,
                    on: \ProfileOrganizationRole.$organization.$id == \Organization.$id
                )
...
struct OrganizationController: RouteCollection {
    ...
    func delete(req: Request) async throws -> HTTPStatus {
        ...
        guard
            let organizationId = req.parameters.get("organizationID").flatMap({
                UUID(uuidString: $0)
            })
        else {
            throw Abort(.badRequest)
    ...
    func putOrganizationMembership(req: Request) async throws -> OrganizationMemberDTO {
            ...
            .first()
        {
            ...
            if oldRole != currentRole.role {
                ...
                await req.trackAnalyticsEvent(
                    ...
                    params: [
                        ...
                        "member_email": currentRole.profile.email,
                        "member_role": update.role.rawValue,
                    ])
            }
            ...
            if let profileToAdd = try await Profile.query(on: req.db).filter(
                \.$email == update.email
            ).first() {
                ...
                await req.trackAnalyticsEvent(
                    ...
                    params: [
                        "organization_id": organizationId.uuidString,
                        "member_email": profileToAdd.email, "member_role": update.role.rawValue,
                    ])
                ...
                try await req.services.emailService.sendEmail(
                    to: update.email,
                    subject: "You've been ivited to \(organization.name)",
                    markdown: emailBody)

                return OrganizationMemberDTO(
                    email: update.email, role: update.role, status: .joined)

            } else {

                if let invitation = try await OrganizationInvite.query(on: req.db).filter(
                    \.$email == update.email
                ).first() {
                    ...
                    return OrganizationMemberDTO(
                        email: update.email, role: update.role, status: .invited)

                } else {

                    let invitation = try OrganizationInvite(
                    ...
                    await req.trackAnalyticsEvent(
                        ...
                        params: [
                            "organization_id": organizationId.uuidString,
                            "member_email": update.email, "member_role": update.role.rawValue,
                        ])
